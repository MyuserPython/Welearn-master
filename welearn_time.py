...
def login(user, pwd):
    ...
    #         print("错误返回,登录失败！")
    #         exit(0)
    welearn.login(user, pwd)
# ---------以上修改---------------------


def get_target_course_info(session):
    global cid, uid, classid, courseInfo

    # session = welearn.login()
    # get course list
...
def output_results():
    ...
    print(dedent('''\
        **********  Created By Avenshy & SSmJaE  **********
           ...
           原作者https://www.github.com/Avenshy/WELearnToSleeep
                        仅供学习，勿滥用
        ***************************************************
    '''))
    print("\n\n")
...
async def simulate(learningTime: int, chapter: Dict, session):
...
async def watcher(session):
    global maxLearningTime

    while True:
        get_target_course_info(session)
        choose_unit()
        input_time()
        ...
        for unit in range(startIndex, endIndex):
            ...
            for chapter in response.json()['info']:
                ...
                tasks.append(asyncio.create_task(simulate(learningTime, chapter, session)))
