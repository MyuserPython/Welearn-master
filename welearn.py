...
def login(username, password, loginmode):
...
def welearn_accuracy_run(session):
    while True:
        # 查询课程信息
    ...
    **********  有问题请邮件联系2983928331@qq.com  **********""")
    # **********  有问题请邮件联系hhy5562877@163.com  **********""")
    input("按任意键退出")


def run_mode(selected='0'):
    loginmode = input('请选择登录方式: \n  1.账号密码登录\n  2.Cookie登录\n\n请输入数字1或2: ')
    printline()
    username = input('请输入账号: ')
    password = input('请输入密码: ')

    session = login(username, password, loginmode)

    if selected == '1':
        welearn_accuracy_run(session)
